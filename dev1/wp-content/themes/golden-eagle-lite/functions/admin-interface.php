...
function goldeneagle_optionsframework_add_admin() {
    global $query_string;
    
    $themename =  goldeneagle_get_option('of_themename');      
    $shortname =  goldeneagle_get_option('of_shortname'); 
    ...
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
			...
			goldeneagle_reset_options($options,'optionsframework');
...
$of_page = add_theme_page($themename,'Theme Options','edit_theme_options', 'optionsframework', 'goldeneagle_optionsframework_options_page', 'div');
	
	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page", 'goldeneagle_load_only');
} 
add_action('admin_menu', 'goldeneagle_optionsframework_add_admin');
/*-----------------------------------------------------------------------------------*/
/* Options Framework Reset Function - of_reset_options */
/*-----------------------------------------------------------------------------------*/
function goldeneagle_reset_options($options,$page = ''){
	global $wpdb;
	$count = 0;
	...
	if($page == 'optionsframework'){
		goldeneagle_delete_option( 'of_options' );
	}
}
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - optionsframework_options_page */
/*-----------------------------------------------------------------------------------*/
function goldeneagle_optionsframework_options_page(){
    $options =  goldeneagle_get_option('of_template');      
    $themename =  goldeneagle_get_option('of_themename');
?>
...
<div class="wrap" id="of_container">
  ...
  <?php wp_nonce_field('theme-update-option'); ?>
    <div id="header">
      ...
      <a href="http://www.goldeneagle.com" target="_new">
    ...
    <?php 
        ...
        $return = goldeneagle_optionsframework_machine($options);
        ?>
    <div id="main">
      <div id="of-nav">
        <ul>
...
/*-----------------------------------------------------------------------------------*/
function goldeneagle_load_only() {
	add_action('admin_head', 'of_admin_head');
	
	wp_enqueue_script('jquery-ui-core');
	wp_register_script('jquery-input-mask', get_stylesheet_directory_uri().'/functions/js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	
	function of_admin_head() { 
			
		echo '<link rel="stylesheet" type="text/css" href="'. get_stylesheet_directory_uri().'/functions/admin-style.css" media="screen" />';
		
		 // COLOR Picker ?>
<link rel="stylesheet" media="screen" type="text/css" href="<?php echo get_stylesheet_directory_uri(); ?>/functions/css/colorpicker.css" />
<script type="text/javascript" src="<?php echo get_stylesheet_directory_uri(); ?>/functions/js/colorpicker.js"></script>
<script type="text/javascript" language="javascript">
		jQuery(document).ready(function(){
			
			
			//Color Picker
			<?php $options = goldeneagle_get_option('of_template');
			
			foreach($options as $option){ 
			if($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border'){
				if($option['type'] == 'typography' OR $option['type'] == 'border'){
					$option_id = $option['id'];
					$temp_color = goldeneagle_get_option($option_id);
					$option_id = $option['id'] . '_color';
					$color = $temp_color['color'];
				}
				else {
					$option_id = $option['id'];
					$color = goldeneagle_get_option($option_id);
				}
				?>
...
<?php
		//AJAX Upload
		?>
<script type="text/javascript" src="<?php echo get_stylesheet_directory_uri(); ?>/functions/js/ajaxupload.js"></script>
...
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - goldeneagle_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'goldeneagle_ajax_callback');
function goldeneagle_ajax_callback() {
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
				...
				goldeneagle_update_option( $clickedID , $uploaded_file['url'] );
	...
	elseif ($save_type == 'options' OR $save_type == 'framework') {
		...
		foreach($options as $option_array){
			$id = $option_array['id'];
			$old_value = goldeneagle_get_option($id);
			$new_value = '';
			...
			if(isset($option_array['id'])) { // Non - Headings...
					...
					if ( is_array($type)){
						foreach($type as $array){
							if($array['type'] == 'text'){
								...
								goldeneagle_update_option( $id, stripslashes($new_value));
							}
						}                 
					}
					elseif($new_value == '' && $type == 'checkbox'){ // Checkbox Save
						
						goldeneagle_update_option($id,'false');
					}
					elseif ($new_value == 'true' && $type == 'checkbox'){ // Checkbox Save
						
						goldeneagle_update_option($id,'true');
					}
					elseif($type == 'multicheck'){ // Multi Check Save
						...
						foreach ($option_options as $options_id => $options_value){
							...
							if(!isset($output[$multicheck_id])){
							  goldeneagle_update_option($multicheck_id,'false');
							}
							else{
							   goldeneagle_update_option($multicheck_id,'true'); 
							}
						}
					} 
					elseif($type == 'typography'){
						...
						goldeneagle_update_option($id,$typography_array);
					...
					elseif($type == 'border'){
						...
						goldeneagle_update_option($id,$border_array);
...
/*-----------------------------------------------------------------------------------*/
function goldeneagle_optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'text':
			$val = $value['std'];
			$std = goldeneagle_get_option($value['id']);
			if ( $std != "") { $val = $std; }
		...
		break;
			...
			$select_value = goldeneagle_get_option($value['id']);
		...
		case "upload":
			$value['std'] = '';
			if (isset($value['std'])){
			$output .= goldeneagle_optionsframework_uploader_function($value['id'],$value['std'],null);
			}
		break;
		case "upload_min":
			
			$output .= goldeneagle_optionsframework_uploader_function($value['id'],$value['std'],'min');
			
		break;
		case "color":
			$val = $value['std'];
			$stored  = goldeneagle_get_option( $value['id'] );
		...
		case "typography":
			...
			$default = $value['std'];
			$typography_stored = goldeneagle_get_option($value['id']);
		...
		case "border":
			...
			$default = $value['std'];
			$border_stored = goldeneagle_get_option( $value['id'] );
		...
		if ( is_array($value['type'])) {
			foreach($value['type'] as $array){
					...
					$id = $array['id']; 
					$std = $array['std'];
					$saved_std = goldeneagle_get_option($id);
...
/*-----------------------------------------------------------------------------------*/
/* OptionsFramework Uploader - goldeneagle_optionsframework_uploader_function */
/*-----------------------------------------------------------------------------------*/
function goldeneagle_optionsframework_uploader_function($id,$std,$mod){
	...
	$uploader = '';
    $upload = goldeneagle_get_option($id);
	
	if($mod != 'min') { 
			$val = $std;
            if ( goldeneagle_get_option( $id ) != "") { $val = goldeneagle_get_option($id); }
